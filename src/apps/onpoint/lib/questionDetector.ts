...
export class QuestionDetector {
  ...
  private readonly BUFFER_MAX = 5

  process(transcript: string): { isQuestion: boolean; text: string } {
    ...
    if (this.buffer.length >= 2) {
      ...
      for (const pattern of QUESTION_PATTERNS) {
        if (pattern.test(this.buffer[this.buffer.length - 1]) || pattern.test(combined)) {
          return { isQuestion: true, text: combined }
        }
      }
    }
