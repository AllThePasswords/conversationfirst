...
const GOOGLE_FONTS_CSS = "https://fonts.googleapis.com/css2?family=Alegreya:ital,wght@0,400;0,700;1,400&family=Bitter:ital,wght@0,400;0,600;0,700;1,400&family=Crimson+Pro:ital,wght@0,400;0,600;0,700;1,400&family=DM+Sans:ital,wght@0,400;0,600;0,700;1,400&family=Fira+Code:wght@400;600&family=Fraunces:ital,opsz,wght@0,9..144,400;0,9..144,600;0,9..144,700;1,9..144,400&family=IBM+Plex+Mono:wght@400;600&family=IBM+Plex+Sans:ital,wght@0,400;0,600;0,700;1,400&family=Inconsolata:wght@400;600&family=JetBrains+Mono:wght@400;600&family=Lora:ital,wght@0,400;0,600;0,700;1,400&family=Merriweather:ital,wght@0,400;0,700;1,400&family=Newsreader:ital,opsz,wght@0,6..72,400;0,6..72,600;0,6..72,700;1,6..72,400&family=Outfit:wght@400;600;700&family=Plus+Jakarta+Sans:ital,wght@0,400;0,600;0,700;1,400&family=Recursive:wght@400;600;700&family=SUSE:wght@400;600;700&family=Source+Code+Pro:wght@400;600&family=Source+Serif+4:ital,opsz,wght@0,8..60,400;0,8..60,600;0,8..60,700;1,8..60,400&family=Spectral:ital,wght@0,400;0,600;0,700;1,400&display=swap";
...
function FontCard({ font, selected, onClick, previewText, previewStyle }) {
  ...
  return (
    <button onClick={onClick} style={{
      ...
      border:on?"2px solid #1a1a1a":"1px solid #e4e2dd",borderRadius:"10px",
      background:on?"#faf9f7":"#fff",cursor:"pointer",transition:"all 0.12s",
      boxShadow:on?"0 2px 12px rgba(0,0,0,0.06)":"none",
    }}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:"4px"}}>
        <span style={{fontFamily:font.family||"inherit",fontSize:"12px",fontWeight:600,color:"#1a1a1a"}}>{font.name}</span>
        {font.style&&<span style={{fontSize:"9px",textTransform:"uppercase",letterSpacing:"0.08em",color:"#bbb",fontWeight:500}}>{font.style}</span>}
      </div>
      <div style={{fontFamily:font.family||"inherit",fontSize:"18px",lineHeight:1.3,color:"#2a2a2a",marginBottom:"4px",...previewStyle}}>
        {previewText||"The quick brown fox jumps over the lazy dog"}
      </div>
      <div style={{fontSize:"11px",color:"#aaa",lineHeight:1.3}}>{font.desc}</div>
    </button>
  );
}

/* Processing animation component */
function ProcessingDemo({ choices }) {
  ...
  return (
    <div style={{ display:"flex", alignItems:"center", gap:"10px" }}>
      <div style={{ width:"2px", height:"16px", background:"#3d6b5e", borderRadius:"1px", animation:"blink 1s ease-in-out infinite" }} />
      <span style={{ fontFamily:m?.family, fontSize:"12px", color:"#a09e99", transition:"opacity 0.3s", letterSpacing:"0.01em" }}>
        {labels[phase]}
        <span style={{ animation:"dot1 1.4s ease-in-out infinite" }}>.</span>
        <span style={{ animation:"dot2 1.4s ease-in-out infinite 0.2s" }}>.</span>
        <span style={{ animation:"dot3 1.4s ease-in-out infinite 0.4s" }}>.</span>
      </span>
    </div>
  );
}
...
function MiniPreview({ choices }) {
  ...
  const cite = (n, href) => ({ display:"inline-flex",alignItems:"center",justifyContent:"center",fontFamily:m.family,fontSize:"9px",fontWeight:600,width:"16px",height:"16px",borderRadius:"8px",background:"#eef4f1",color:"#3d6b5e",border:"1px solid rgba(61,107,94,0.25)",verticalAlign:"super",margin:"0 1px",top:"-1px",position:"relative",textDecoration:"none",cursor:"pointer",transition:"all 0.12s" });
  return (
    <div style={{fontFamily:b.family,fontSize:"14px",lineHeight:1.55,color:"#2a2a2a"}}>
      {/* Processing state */}
      <div style={{marginBottom:"16px",padding:"12px 16px",background:"#fff",border:"1px solid #e4e2dd",borderRadius:"10px"}}>
        <div style={{fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.08em",color:"#bbb",fontWeight:600,marginBottom:"8px"}}>Processing</div>
        <ProcessingDemo choices={choices} />
      </div>

      {/* Answer */}
      <div style={{padding:"12px 16px",background:"#fff",border:"1px solid #e4e2dd",borderRadius:"10px"}}>
        <div style={{fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.08em",color:"#bbb",fontWeight:600,marginBottom:"8px"}}>Response</div>
        <p style={{marginBottom:"8px"}}>
          124% net revenue retention. Up from 118% in Q2<a href="https://docs.google.com/document/d/example" target="_blank" style={cite()}>1</a>.
        </p>
        <p style={{marginBottom:"10px",color:"#666",fontSize:"13px"}}>
          Industry median is 110%<a href="https://openviewpartners.com/benchmarks" target="_blank" style={cite()}>2</a>. Improvement from mid-market expansion where ACV increased 31%<a href="https://docs.google.com/document/d/example" target="_blank" style={cite()}>1</a>.
        </p>
        <div style={{display:"flex",gap:"8px",padding:"8px 12px",background:"#eef4f1",borderLeft:"3px solid #3d6b5e",borderRadius:"0 6px 6px 0",fontSize:"11px",marginBottom:"10px"}}>
          <span style={{fontFamily:m.family,fontSize:"9px",fontWeight:600,color:"#3d6b5e"}}>1</span>
          <div>
            <div style={{fontWeight:600,fontSize:"11px"}}>Q3 Financial Report</div>
            <div style={{fontSize:"9px",color:"#999"}}>Google Drive · Document</div>
          </div>
        </div>
        <div style={{borderTop:"1px solid #e8e6e1",paddingTop:"8px",fontSize:"10px",color:"#999"}}>
          <span style={{fontFamily:m.family,fontWeight:600,color:"#3d6b5e",marginRight:"4px"}}>1</span> Q3 Report
          <span style={{margin:"0 6px"}}>·</span>
          <span style={{fontFamily:m.family,fontWeight:600,color:"#3d6b5e",marginRight:"4px"}}>2</span> SaaS Benchmarks
        </div>
      </div>

      {/* Components */}
      <div style={{marginTop:"14px",display:"flex",gap:"6px",flexWrap:"wrap",alignItems:"center"}}>
        <span style={{fontSize:"11px",fontWeight:600,color:"#fff",background:"#3d6b5e",padding:"4px 12px",borderRadius:"6px"}}>Primary</span>
        <span style={{fontSize:"11px",fontWeight:600,color:"#2a2a2a",background:"#fff",padding:"4px 12px",borderRadius:"6px",border:"1px solid #e4e2dd"}}>Secondary</span>
        <span style={{fontSize:"9px",fontWeight:600,background:"#eef4f1",color:"#3d6b5e",padding:"2px 8px",borderRadius:"99px"}}>Badge</span>
        <span style={{fontFamily:m.family,fontSize:"11px",background:"#f3f1ed",padding:"2px 6px",borderRadius:"3px",color:"#666"}}>code</span>
      </div>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <style>{`
        ...
        .fin{animation:fadeIn 0.2s ease-out}
      `}</style>
      <div style={{minHeight:"100vh",background:"#faf9f7",fontFamily:"'DM Sans', system-ui, sans-serif"}}>
        {toast&&<div style={{position:"fixed",bottom:"24px",left:"50%",transform:"translateX(-50%)",background:"#1a1a1a",color:"#fff",padding:"9px 22px",borderRadius:"8px",fontSize:"12px",fontWeight:600,zIndex:100,animation:"toastUp 0.18s ease-out",boxShadow:"0 4px 20px rgba(0,0,0,0.15)"}}>{toast}</div>}

        <div style={{borderBottom:"1px solid #e4e2dd",background:"#fff"}}>
          <div style={{maxWidth:"1000px",margin:"0 auto",padding:"18px 32px"}}>
            <div style={{fontSize:"10px",textTransform:"uppercase",letterSpacing:"0.1em",color:"#bbb",marginBottom:"2px",fontWeight:500}}>Conversation First</div>
            <h1 style={{fontSize:"20px",fontWeight:700,color:"#1a1a1a",letterSpacing:"-0.025em",margin:0}}>App Configuration</h1>
          </div>
        </div>

        <div style={{maxWidth:"1000px",margin:"0 auto",padding:"16px 32px 0"}}>
          <div style={{display:"flex",gap:"3px",marginBottom:"8px"}}>
            {STEPS.map((s,i)=>(
              <button key={s} onClick={()=>{if(i<=step||(i===3&&all))setStep(i)}}
                style={{flex:1,height:"3px",borderRadius:"2px",background:i<=step?"#1a1a1a":"#e0ddd8",border:"none",cursor:i<=step||(i===3&&all)?"pointer":"default",transition:"background 0.2s",padding:0}}/>
            ))}
          </div>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"18px"}}>
            <span style={{fontSize:"12px",color:"#999"}}>Step {step+1}/4 — {STEP_TITLES[cur]}</span>
            <div style={{display:"flex",gap:"8px"}}>
              {step>0&&<button onClick={()=>setStep(step-1)} style={{fontSize:"12px",color:"#999",background:"none",border:"none",cursor:"pointer",textDecoration:"underline",padding:0}}>Back</button>}
              {canNext&&<button onClick={()=>setStep(step+1)} style={{fontSize:"12px",color:"#1a1a1a",background:"#e8e6e1",border:"none",borderRadius:"6px",padding:"5px 14px",cursor:"pointer",fontWeight:600}}>Next →</button>}
            </div>
          </div>
        </div>

        <div style={{maxWidth:"1000px",margin:"0 auto",padding:"0 32px 72px"}}>
          {(cur==="body"||cur==="heading"||cur==="mono")&&(
            <div className="fin" key={cur}>
              <p style={{fontSize:"13px",color:"#888",marginBottom:"16px",lineHeight:1.5,maxWidth:"520px"}}>{STEP_DESCS[cur]}</p>
          ...
          {cur==="review"&&all&&(
            <div className="fin" key="review">
              <div style={{display:"flex",gap:"8px",flexWrap:"wrap",marginBottom:"16px"}}>
                <button onClick={()=>{dl(generateSpec(choices),"conversation-first-spec.md","text/markdown");flash("Spec downloaded")}}
                  style={{fontSize:"13px",fontWeight:600,color:"#fff",background:"#1a1a1a",border:"none",borderRadius:"8px",padding:"10px 20px",cursor:"pointer"}}>
                  Download spec (.md)
                </button>
                <button onClick={()=>{dl(generateTestPage(choices),"test-page.html","text/html");flash("Test page downloaded")}}
                  style={{fontSize:"13px",fontWeight:600,color:"#1a1a1a",background:"#e8e6e1",border:"none",borderRadius:"8px",padding:"10px 20px",cursor:"pointer"}}>
              ...
              <p style={{fontSize:"12px",color:"#aaa",marginBottom:"24px",lineHeight:1.5,maxWidth:"540px"}}>
          ...
          {cur==="review"&&!all&&(
            <div style={{textAlign:"center",padding:"48px",color:"#bbb"}}>
              ...
              <button onClick={()=>setStep(0)} style={{marginTop:"12px",fontSize:"12px",color:"#1a1a1a",background:"#e8e6e1",border:"none",borderRadius:"8px",padding:"8px 20px",cursor:"pointer"}}>Start over</button>
            </div>
          )}
        </div>
      </div>
